...
import { checkSubscriptionActive } from "../../../utils/services/smartContract";

const getData = async (setSubdomain, setInputTerm) => {
  axios
    .get(`${baseUrl}/api/gateway/get_subdomain?publicKey=${getAddress()}`)
    .then(
      (response) => {
...
const createGateway = async (value, transaction) => {
  if (transaction) {
    axios
      .post(`${baseUrl}/api/gateway/add_subdomain`, {
...
const checkSubdomain = async (value, setDomainAvailable) => {
  axios.get(`${baseUrl}/api/gateway/check_subdomain?subDomain=${value}`).then(
    (response) => {
...
const checkTransaction = async (setTransaction) => {
  axios
    .get(
      `${baseUrl}/api/gateway/get_transaction_details?publicKey=${getAddress()}`
    )
    .then(
      (response) => {
...
function Gateway() {
  ...
  useEffect(() => {
    ...
    // (async () => {
    //   let data = null;
    //   data = await checkSubscriptionActive();
    //   console.log(data);
    // })();
    return () => {};
  }, []);
    ...
    <div className="gateway">
      <div className="gatewayMain">
        ...
        <div className="gatewayMain__content">
          <p>
            {subdomain
              ? "Your Custom IPFS Gateway"
              : "Enter your custom IPFS Gateway at lighthouse"}
          </p>
          ...
          <div className="input-box">
            ...
            />
            <span className="suffix">.lighthouse.storage</span>
          </div>

          {subdomain !== inputTerm && domainAvailable !== null && (
          ...
          {subdomain && (
            <p className="subdomain">
              Your lighthouse Gateway is &nbsp;
              <span
                className="link"
                onClick={() => {
                  copyToClipboard(subdomain + ".lighthouse.storage");
                }}
              >{`${subdomain}.lighthouse.storage`}</span>
            </p>
          )}
          <button
            disabled={
            ...
            onClick={() => {
              transaction
                ? createGateway(inputTerm, transaction)
                : createTransaction(inputTerm, setTxHash, transaction);
            }}
          >
            {subdomain ? (
              <span>Update IPFS Gateway</span>
            ) : (
              <span>Create IPFS Gateway ( 5 USDC )</span>
            )}
          </button>
        </div>
      </div>
